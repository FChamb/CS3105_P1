...
public class AStarSearchOpt {
    ...
    public int search() {
        ...
        Node begin = new Node(null, start[0], start[1], 0, 1, calculateHeuristic(coverage, 1, start[0], start[1]));
        frontier.add(begin);
     ...
     */
    private int calculateHeuristic(int coverage, int visited, int col, int row) {
        int remainingCells = coverage - visited;
        int heuristic = 3 * remainingCells;

        // Add a slight preference for horizontal or vertical moves
        if (Math.abs(col - this.start[0]) % 3 == 0 || Math.abs(row - this.start[1]) % 3 == 0) {
            heuristic -= 2; // Subtract to give preference to straight moves
        }
        return heuristic;
    }
    ...
    public int calculateCost(int[] move) {
        ...
        return 5; // Increase cost for diagonal moves
    }
